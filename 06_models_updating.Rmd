...
# reaction time - model for updating
...
# model
```{r}
# filtering for bins 4-6
rt_ls_updating <- rt_ls %>% 
  filter(bin == 4 | bin == 5 | bin == 6) %>% 
  mutate(bin = relevel(bin, ref = 4))

# fitting model
rt_ls_updating_model <- 
  mixed(ls_binwise_rt ~ bin * aq_centered + (1 | id), data = rt_ls_updating, method = "S", check_contrasts = F)
rt_ls_updating_model
summary(rt_ls_updating_model$full_model)

tab_model(rt_ls_updating_model$full_model,
          df.method = "s",
          show.se = F,
          show.stat = T,
          show.df = T,
...
emm_options(lmer.df = "Satterthwaite", lmerTest.limit = 290682)
  ...
  ## bin main effet
  emm_bin <- emmeans(rt_ls_updating_model, ~ bin, type = 'response')
  emm_bin
  pairs(regrid(emm_bin), adjust = "sidak")

# assumptions
car::qqPlot(resid(rt_ls_updating_model$full_model))
plot(rt_ls_updating_model$full_model)
plot_model(rt_ls_updating_model$full_model, type = "diag")
check_normality(rt_ls_updating_model$full_model)
check_collinearity(rt_ls_updating_model$full_model)
```

# accuracy - model for updating
## converting bin variable to factor
```{r}
acc_ls <- acc_ls %>% 
  mutate(
